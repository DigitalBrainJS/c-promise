...
const CPromise= class CPromise extends Promise {
     ...
     * @param {CPFinallyHandler} finallyHandler
     * @returns {Promise<T | void>}
     */

    finally(finallyHandler) {
        return this.then((value, scope) => finallyHandler.call(scope, value, false, scope),
          (err, scope) => finallyHandler.call(scope, err, true, scope))
    }
     ...
     * @param {object} [options.innerWeight]
     * @param {object} [options.weight]
...
const decorators = {
    async: propertyDecorator((decorator, [options], {context}) => {
        ...
        const {timeout, label, innerWeight, weight, listen, scopeArg= false} = options || {};
    ...
    ReactComponent: classDecorator((descriptor, options, {context}) => {
        const {subscribeAll}= options || {};
        const descriptors= {};
        let propsChanged;

        const reactProps= ['componentWillUnmount', 'componentDidMount']
            ...
            finisher(constructor) {
                ...
                Object.getOwnPropertyNames(prototype).forEach(prop => {
                    ...
                    if (isGeneratorFunction(method)) {
                        asyncMethod = context.promisify(method, {scopeArg: reactProps.indexOf(prop)!==-1 });
                    ...
                    if (prop === 'componentWillUnmount') {
                        decoratedMethod = function () {
                            ...
                            return asyncMethod.apply(this, arguments);
                        }
                    } else if (prop === 'componentDidMount' || subscribeAll) {
                        decoratedMethod = function () {
                            return asyncMethod
