...
const CPromise= class CPromise extends Promise {
    ...
    static delay(ms, value, options) {
        return new this((resolve, reject, scope) => {
            ...
            scope[_render]= ()=> `${scope.constructor.name}.delay(${ms})`;
    ...
    static all(iterable, options) {
        return new this((resolve, reject, scope) => {
            ...
            if (!concurrency) {
                ...
                scope[_render]= ()=> `${scope.constructor.name}.all[${pending.length}]`;

                return super.all(pending).then(resolve, _reject);
            }

            if (isArray(iterable)) {
                scope[_render]= ()=> `${scope.constructor.name}.all[${iterable.length}]`;
    ...
    static race(pending) {
        return new this((resolve, reject, scope) => {
            ...
            scope[_render]= () => `${scope.constructor.name}.race[${length}]`;
    ...
    static allSettled(iterable, options) {
        ...
        promise[_render] = ()=> `${scope.constructor.name}.allSettled`;
    ...
    static retry(fn, options) {
        ...
        return new this((resolve, reject, scope) => {
            ...
            const attempt = () => {
                ...
                scope[_render]= ()=> `${scope.constructor.name}.try[${attemptIndex}/${retries}]`;
    ...
    static run(generatorFn, {args, resolveSignatures, context, scopeArg= false} = {}) {
        return new this((resolve, reject, scope) => {
            ...
            scope[_render] = ()=> `${scope.constructor.name}.run${name ? '(' + name + '*)' : ''}`;
    ...
    toString(entireChain){
        ...
        const renderScope= (scope)=>{
            ...
            const title= label || (render && render.call(scope)) || scope.constructor.name;
